const AuV = artifacts.require("AuVImplementation.sol");
...
// Tests that AuV Asset Protection capabilities function correctly.
contract(
  "AuV",
  function ([
    _,
    ...
    beforeEach(async function () {
      const auv = await AuV.new({ from: owner });
    ...
    describe("as a asset-protection token", function () {
      ...
      describe("freeze", function () {
        ...
        it("adds the frozen address", async function () {
          ...
          const isFrozen = await this.token.isFrozen(freezableAddress, {
            from: assetProtectionRole,
          });
        ...
        it("reverts when address is already frozen", async function () {
          ...
          await assertRevert(
            this.token.freeze(freezableAddress, { from: assetProtectionRole })
          );
        });
      });
      ...
      describe("unfreeze", function () {
        it("reverts when address is already unfrozen", async function () {
          await assertRevert(
            this.token.unfreeze(freezableAddress, { from: assetProtectionRole })
          );
        });
      ...
      describe("setAssetProtectionRole", function () {
        ...
        it("reverts if sender is not owner or AssetProtectionRole", async function () {
          await assertRevert(
            this.token.setAssetProtectionRole(otherAddress, {
              from: otherAddress,
            })
          );
        });
        ...
        it("works if sender is AssetProtectionRole", async function () {
          await this.token.setAssetProtectionRole(otherAddress, {
            from: assetProtectionRole,
          });
        ...
        it("emits a AssetProtectionRoleSet event", async function () {
          const { logs } = await this.token.setAssetProtectionRole(
            otherAddress,
            { from: assetProtectionRole }
          );
          ...
          assert.equal(
            logs[0].args.oldAssetProtectionRole,
            assetProtectionRole
          );
