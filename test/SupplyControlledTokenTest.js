...
// Tests that AuV token supply control mechanisms operate correctly.
contract(
  "AuV",
  function ([_, admin, newSupplyController, otherAddress, owner]) {
    ...
    describe("as a supply-controlled token", function () {
      ...
      describe("setSupplyController", function () {
        ...
        beforeEach(async function () {
          const res = await this.token.setSupplyController(
            newSupplyController,
            {
              from: owner,
            }
        ...
        it("enables new supply controller to increase and decrease supply", async function () {
          ...
          await this.token.decreaseSupply(amount, {
            from: newSupplyController,
          });
          balance = await this.token.balanceOf(newSupplyController);
        ...
        it("prevents old supply controller from increasing and decreasing supply", async function () {
          await assertRevert(
            this.token.increaseSupply(amount, { from: owner })
          );
