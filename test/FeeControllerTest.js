...
// Tests that AuV fee controller capabilities function correctly.
contract(
  "Fee Controller AuV",
  function ([
    _,
    ...
    beforeEach(async function () {
      ...
      await proxiedAuV.initializeBalance(owner, ownerStartingBalance);
    ...
    describe("when the contract is first deployed", function () {
      ...
      it("sends a fee of 0 for a 10000 tansferFrom", async function () {
        await this.token.approve(otherAddress, ownerStartingBalance, {
          from: owner,
        });
        const amount = 10000;
        const expectedFee = 0;
        await this.token.transferFrom(owner, recipient, amount, {
          from: otherAddress,
        });
        const senderBalance = await this.token.balanceOf(owner);
        assert.equal(senderBalance, ownerStartingBalance - amount);
    ...
    describe("contract fee permissions", function () {
      ...
      it("can set fee rate by feeController", async function () {
        const { logs } = await this.token.setFeeRate(feeRate, {
          from: feeController,
        });
        let currentFeeRate = await this.token.feeRate();
      ...
      it("can set fee controller to new address by feeController", async function () {
        const { logs } = await this.token.setFeeController(otherAddress, {
          from: feeController,
        });
        let currentFeeController = await this.token.feeController();
      ...
      it("can set fee recipient to new address by feeController", async function () {
        const { logs } = await this.token.setFeeRecipient(feeRecipient, {
          from: feeController,
        });
      ...
      it("cannot set fee recipient to new address by contract owner", async function () {
        await assertRevert(
          this.token.setFeeRecipient(feeRecipient, { from: owner })
        );
      });
      ...
      it("errors when fee rate is set by unauthorized address", async function () {
        await assertRevert(
          this.token.setFeeRate(feeRate, { from: otherAddress })
        );
      });
      it("errors when fee controller is set by unauthorized address", async function () {
        await assertRevert(
          this.token.setFeeController(otherAddress, { from: otherAddress })
        );
      });
      it("errors when fee recipient is set by unauthorized address", async function () {
        await assertRevert(
          this.token.setFeeRecipient(otherAddress, { from: otherAddress })
        );
      });
      it("cannot set fee controller to 0 address", async function () {
        await assertRevert(
          this.token.setFeeController(zeroAddress, { from: owner })
        );
      });
      it("cannot set fee recipient to 0 address", async function () {
        await assertRevert(
          this.token.setFeeRecipient(zeroAddress, { from: owner })
        );
      });
    });
    ...
    describe("fee rate tests", function () {
      ...
      it("cannot have a fee rate of over 1000000 fee parts", async function () {
        ...
        await assertRevert(
          this.token.setFeeRate(feeRate, { from: feeController })
        );
      });
    });
    ...
    describe("transfer fee tests", function () {
      ...
      describe("basic fees on transfers", function () {
        beforeEach(async function () {
          ...
          await this.token.setFeeRecipient(feeRecipient, {
            from: feeController,
          });
          await this.token.setFeeRate(feeRate, { from: feeController });
          await this.token.topupBalance(owner, ownerStartingBalance);
        ...
        tests.forEach((test) => {
          it(test.name("transfer"), async function () {
            const { logs } = await this.token.transfer(recipient, test.amount, {
              from: owner,
            });
            const senderBalance = await this.token.balanceOf(owner);
      ...
      describe("basic fees on betaDelegatedTransfers", function () {
        ...
        beforeEach(async function () {
          ...
          let { receipt } = await this.token.topupBalance(
            fromAddress,
        ...
        tests.forEach((test) => {
          it(test.name("betaDelegatedTransfer"), async function () {
            ...
            const sig = ethSigUtil.signTypedData(privateKey, {
              data: typedData,
            });
            ...
            assert.equal(
              senderBalance,
              ownerStartingBalance - (test.amount + serviceFeeAmount)
            );
            let executorBalance = await this.token.balanceOf(executor);
            ...
            assert.equal(
              logs[logLength - 1].args.value,
              test.amount - test.expectedFee
            );
